/**
 * Heat Shield — "Liquid Glass V2" Help page (lg2-native).
 *
 * Full scope of the v1 `HelpTab` (tabs/help.tsx): every function at a glance
 * with a short bilingual explanation, grouped by area and filterable via a
 * search box (incl. mode + convention explanations). Rebuilt in a dedicated
 * lg2 layout with its own `lg2-help-*` classes and only `--lg2-*` tokens.
 */
...
export function LiquidGlass2Help(_props: RoutableProps): JSX.Element {
  ...
  return (
    <main class="lg2-main lg2-help" data-testid="liquid-glass2-help">
      <header class="lg2-header">
        <div>
          <h1 class="lg2-header__title">{t('Hilfe', 'Help')}</h1>
          <p class="lg2-header__sub">{t('Funktionen im Überblick', 'Features at a glance')}</p>
        </div>
      </header>

      <p class="lg2-help-intro">
        {t(
      ...
      <div class="lg2-help-searchbox">
        <Icon name="einstellungen" size={18} class="lg2-help-searchbox__icon" />
        <input
          type="search"
          class="lg2-help-search"
          data-testid="lg2-help-search"
      ...
      {groups.length === 0 ? (
        <p class="lg2-help-empty">{t(`Kein Treffer für „${query}".`, `No match for "${query}".`)}</p>
      ) : (
        <div class="lg2-help-groups">
          {groups.map((g) => (
            <article key={g.title} class="lg2-card lg2-help-group" data-testid="lg2-help-group">
              <h3 class="lg2-help-group__title">{g.title}</h3>
              <dl class="lg2-help-group__list">
                {g.items.map((it) => (
                  <div key={it.name} class="lg2-help-group__row">
                    <dt class="lg2-help-group__name">{it.name}</dt>
                    <dd class="lg2-help-group__desc">{it.desc}</dd>
                  </div>
                ))}
              </dl>
            </article>
